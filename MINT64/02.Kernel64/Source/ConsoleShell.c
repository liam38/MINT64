...
// 태스크를 생성해서 멀티태스킹 수행 (Task를 생성해서 ReadyList에 추가하는 함수)
...
static void kChangeTaskPriority(const char* pcParameterBuffer) {
	...
	// 태스크의 우선순위를 변경
	if(kMemCmp(vcID, "0x", 2) == 0) 	// kMemCpy로 했다가 page fault 나서 고생했네..
...
static void kShowTaskList(const char* pcParameterBuffer) {
	...
	for(i = 0; i < TASK_MAXCOUNT; i++) {
		...
		if((pstTCB->stLink.qwID >> 32) != 0) {
			...
			kPrintf("	Parent PID[0x%Q], Memory Address[0x%Q], Size[0x%Q]\n", pstTCB->qwParrentProcessID, pstTCB->pvMemoryAddress, pstTCB->qwMemorySize);
		}
	}
}
...
static void kWriteSector(const char* pcParameterBuffer) {
	...
	// 파라미터 리스트를 초기화하여 LBA 어드레스와 섹터수 추출
	kInitializeParameter(&stList, pcParameterBuffer);
	if((kGetNextParameter(&stList, vcLBA) == 0) || (kGetNextParameter(&stList, vcSectorCount) == 0)) {
...
static void kShowRootDirectory(const char* pcParameterBuffer) {
	...
	// 먼저 루프를 돌면서 디렉터리에 잇는 파일의 개수와 전체 파일이 사용한 크기를 계산
	iTotalCount = 0;
	dwTotalByte = 0;
...
static void kTestFileIO(const char* pcParameterBuffer) {
	...
	// 파일이 열려 잇는 상태이므로 파일을 지우려고 하면 실패해야 함
...
// 파일 시스템의 캐시 버퍼에 잇는 데이터를 모두 하드 디스크에 씀
