...
BOOL kInitializeHDD(void) {
	...
	gs_stHDDManager.bSecondearyInterruptOccur = FALSE;

	// 첫 번째와 두 번째 PATA 포트의 디지털 출력 레지스터 (0x3F6, 0x376)에 0 출력
	...
	// 하드디스크가 검색 되었으면 QEMU에서만 쓸 수 있도록 설정
...
// 하드디스크의 상태를 반환
static BYTE kReadHDDStatus(BOOL bPrimay) {
	if(bPrimay == TRUE) {
		// 첫 번째 PATA 포트의 상태 레지스터 (0x1F7)에서 값을 반환
		return kInPortByte(HDD_PORT_PRIMARYBASE + HDD_PORT_INDEX_STATUS);
	}
	// 두 번째 PATA 포트의 상태 레지스터 (0x177)에서 값을 반환
...
static BOOL kWaitForHDDNoBusy(BOOL bPrimary) {
	...
	while((kGetTickCount() - qwStartTickCount) <= HDD_WAITTIME) {
		...
		// Busy 비트(bit 7)가 설정되어 있지 않으면  Busy 가 해제된 것이므로 종료
...
static BOOL kWaitForHDDReady(BOOL bPrimary) {
	...
	while((kGetTickCount() - qwStartTickCount) <= HDD_WAITTIME) {
		...
		// Ready 비트(bit 6)가 설정되어 있으면 데이터를 받을 준비가 된 것이므로 종료
...
void kSetHDDInterruptFlag(BOOL bPrimary, BOOL bFlag) {
	if(bPrimary == TRUE) {
		...
		gs_stHDDManager.bSecondearyInterruptOccur = bFlag;
	}
}
...
static BOOL kWaitForHDDInterrupt(BOOL bPrimary) {
	...
	while(kGetTickCount() - qwTickCount <= HDD_WAITTIME) {
		// 하드디스크 자료구조에 인터럽트 발생 플래그를 확인
		if((bPrimary == TRUE) && (gs_stHDDManager.bPrimaryInterruptOccur == TRUE)) {
			return TRUE;
		}
		else if((bPrimary == FALSE) && (gs_stHDDManager.bSecondearyInterruptOccur == TRUE)) {
			return TRUE;
		}
	}
...
BOOL kReadHDDInformation(BOOL bPrimary, BOOL bMaster, HDDINFORMATION* pstHDDInformation){
	...
	// 아직 수행중인 커맨드가 있다면 일정 시간 동안 끝날 때까지 대기
	...
	// 커맨드 전송 후 인터럽트 대기
	//=========================================================================
	// 커맨드를 받아들일 준비가 될 때까지 일정시간 동안 대기
...
int kReadHDDSector(BOOL bPrimary, BOOL bMaster, DWORD dwLBA, int iSectorCount, char* pcBuffer) {
	...
	// 아직 수행중인 커맨드가 있다면 일정 시간 동안 끝날 떄까지 대기
	if(kWaitForHDDNoBusy(bPrimary) == FALSE) {
		// 동기화 처리
		kUnlock(&(gs_stHDDManager.stMutex));
		return FALSE;
	...
	//=========================================================================
	// 섹터 수 레지스터(0x1F2 or 0x172)에 읽을 섹터 수 전송
	kOutPortByte(wPortBase + HDD_PORT_INDEX_SECTORCOUNT, iSectorCount);
	// 섹터 번호 레지스터(0x1F3 or 0x173)에 읽을 섹터 위치(LBA 0~7bit) 전송
	kOutPortByte(wPortBase + HDD_PORT_INDEX_SECTORNUMBER, dwLBA);
	// 실린더 LSB 레지스터(0x1F4 or 0x174)에 읽을 섹터 위치(LBA 8~15bit) 전송
	kOutPortByte(wPortBase + HDD_PORT_INDEX_CYLINDERLSB, dwLBA >> 8);
	// 실린더 MSB 레지스터(0x1F5 or 0x175)에 읽을 섹터 위치(LBA 16~23bit) 전송
	...
	// 드라이브/헤드 레지스터(0x1F6 or 0x176)에 읽을 섹터 위치(LBA 24~27bit)와
	// 설정된 값을 같이 전송
	...
	// 커맨드 레지스터(0x1F7 or 0x177)에 읽기(0x20) 전송
	...
	for(i = 0; i < iSectorCount; i++) {
		...
		// DATAREQUEST bit가 설정되지 않았으면 데이터가 수신되기를 기다림
		if((bStatus & HDD_STATUS_DATAREQUEST) != HDD_STATUS_DATAREQUEST) {
			// 처리가 완료될 때까지 일정시간 동안 인터럽트를 기다림
			bWaitResult = kWaitForHDDInterrupt(bPrimary);
			kSetHDDInterruptFlag(bPrimary, FALSE);
...
int kWriteHDDSector(BOOL bPrimary, BOOL bMaster, DWORD dwLBA, int iSectorCount, char* pcBuffer) {
	...
	// 아직 수행 중인 커맨드가 있다면 일정 시간 동안 끝날 떄까지 대기
	if(kWaitForHDDNoBusy(bPrimary) == FALSE) {
		return FALSE;
	}
	...
	// 		LBA 모드는 섹터번호 -> 실린더 번호 -> 헤드 번호의 순으로 LBA 어드레스를 대입
	//=========================================================================
	// 섹터 수 레지스터 (0x1F2 or 0x172)에 쓸 섹터 수 전송
	kOutPortByte(wPortBase + HDD_PORT_INDEX_SECTORCOUNT, iSectorCount);
	// 섹터 번호 레지스터 (0x1F3 or 0x173)에 쓸 섹터 위치(LBA 0~7bit) 전송
	kOutPortByte(wPortBase + HDD_PORT_INDEX_SECTORNUMBER, dwLBA);
	// 실린더 LSB 레지스터 (0x1F4 or 0x174)에 쓸 섹터 위치(LBA 8~15bit) 전송
	kOutPortByte(wPortBase + HDD_PORT_INDEX_CYLINDERLSB, dwLBA >> 8);
	// 실린더 MSB 레지스터 (0x1F5 or 0x175)에 쓸 섹터 위치(LBA 16~23bit) 전송
	kOutPortByte(wPortBase + HDD_PORT_INDEX_CYLINDERMSB, dwLBA >> 16);
	// 드라이브와 헤드 데이터 설정
	if(bMaster == TRUE) {
		bDriveFlag = HDD_DRIVEANDHEAD_LBA;
	...
	// 드라이브/헤드 레지스터 (0x1F6 or 0x176)에 쓸 섹터 위치(LBA 24~27bit)와
	// 설정된 값을 같이 전송
	kOutPortByte(wPortBase + HDD_PORT_INDEX_DRIVEANDHEAD, bDriveFlag | ((dwLBA >> 24) & 0x0F));
	...
	for(i = 0; i < iSectorCount; i++) {
		...
		// DATAREQUEST bit가 설정되지 않았으면 데이터가 처리가 완료되길 기다림
		if((bStatus & HDD_STATUS_DATAREQUEST) != HDD_STATUS_DATAREQUEST) {
