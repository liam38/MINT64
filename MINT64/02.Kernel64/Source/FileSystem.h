...
// MINT 파일 시스템 함수를 표준 힙출력 함수 이름으로 재정의
...
typedef struct kPartitionStruct {
	// 부팅 가능 플래그. 0x80이면 부팅가능, 0x00은 부팅 불가
	BYTE bBootableFlag;
...
// 파일과 디렉터리에 대한 정보가 들어 잇는 자료구조
typedef struct kFileDirectoryHandleSturct {
	// 자료구조의 타입 설정. 파일 핸들이나 디렉터리 핸들, 또는 빈 핸들 타입 지정가능
	BYTE bType;
