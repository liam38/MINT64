...
void kInitializePIT(WORD wCount, BOOL bPeriodic) {
	// PIT control register(0x43)에 값을 초기화하여 count를 멈춘 뒤에
	// 모드 0 에 바이너리 카운터로 설정
	kOutPortByte(PIT_PORT_CONTROL, PIT_COUNTER0_ONCE);

	// 만약 일정한 주기로 반복하는 타이머라면 모드 2로 설정
	if(bPeriodic == TRUE) {
		// PIT control register(0x43)에 모드 2에 바이너리 카운터로 설정
		kOutPortByte(PIT_PORT_CONTROL, PIT_COUNTER0_PERIODIC);
	}

	// 카운터 0(0x40)에 LSB -> MSB 순으로 카운터 초기 값을 설정
...
WORD kReadCounter0(void) {
	...
	// PIT control register(0x43)에 latch 커맨드를 전송하여 카운터 0에 있는 현재 값을 읽음
	kOutPortByte(PIT_PORT_CONTROL, PIT_COUNTER0_LATCH);

	// 카운터 0(0x40)에서 LSB -> MSB 순으로 카운터 값을 읽음
...
// 		함수를 호출하면 PIT 컨트롤러의 설정이 바뀌므로, 이후에 PIT 컨트롤러 재설정해야 함
