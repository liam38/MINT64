...
static TCB* kAllocateTCB(void) {
	...
	for(i = 0; i < gs_stTCBPoolManager.iMaxCount; i++) {
		//ID의 상위 32bit가 0 이면 할당되지 않은 TCB
	...
	// 상위 32bit를 0이 아닌 값으로 설정해서 할당된 TCB로 설정
...
TCB* kCreateTask(QWORD qwFlags, void* pvMemoryAddress, QWORD qwMemorySize, QWORD qwEntryPointAddress) {
	...
	if(qwFlags & TASK_FLAGS_THREAD) {
		...
		pstTask->qwParrentProcessID = pstProcess->stLink.qwID;
	...
	// 스레드의 ID를 Task ID와 동일하게 설정
	pstTask->stThreadLink.qwID = pstTask->stLink.qwID;
	// 임계 영역 끝
	kUnlockForSystemData(bPreviousFlag);

	// 태스크 ID로 스택 어드레스 계산, 하위 32bit가 스택풀의 오프셋 역할 수행
...
void kInitializeScheduler(void) {
	...
	pstTask->qwParrentProcessID = pstTask->stLink.qwID;
...
void kSchedule(void) {
	...
	// 현재 수행중인 태스크의 정보를 수정한 뒤 Context 전환
	pstRunningTask = gs_stScheduler.pstRunningTask;
	...
	// 다음에 수앻할 태스크가 FPU를 쓴 태스크가 아니라면 TS 비트 설정
	if(gs_stScheduler.qwLastFPUUsedTaskID != pstNextTask->stLink.qwID)
		kSetTS();
	else
		kClearTS();

	// 태스크 종료 플래그가 설정된 경우 Context를 저장할 필요가 없으므로, 대기 리스트에 삽입하고 Context 전환
...
// 인터럽트가 발생시 다른 태스크를 찾아 전환
// 		반드시 인터럽트나 예외가 발생했을 때만 호출해야 함
BOOL kScheduleInInterrupt(void) {
	...
	if(pstRunningTask->qwFlags & TASK_FLAGS_ENDTASK)
		kAddListToTail(&(gs_stScheduler.stWaitList), pstRunningTask);
	// 태스크가 종료되지 않으면 IST에 있는 Context를 복사하고, 현재 태스크를 준비 리스트로 옮김
	else {
	...
	// 전환해서 실행할 태스크를 Running Task로 설정하고 Context를 IST에 복사해서
...
int kGetTaskCount(void) {
	...
	// 준비 큐의 태스크 수를 구한 후 대기 큐의 태스크 수와 현재 수행중인 태스크 수를 더함
...
static TCB* kGetProcessByThread(TCB* pstThread) {
	...
	// 프로세스가 아니라면, 부모 프로세스로 설정된 태스크 ID를 통해
	// TCB 풀에서 태스크 자료구조 추출
	pstProcess = kGetTCBInTCBPool(GETTCBOFFSET(pstThread->qwParrentProcessID));

	// 만약 프로세스가 없거나, 태스크 ID가 일치하지 않는다면 NULL을 반환
	if((pstProcess == NULL) || (pstProcess->stLink.qwID != pstThread->qwParrentProcessID))
		return NULL;
...
void kIdleTask(void) {
	...
	while(1) {
		...
		// 프로세서 사용량을 계산
		// 100 - (Idle Task의 프로세서 시간) * 100 / (시스템 전체에서 사용한 프로세서 시간)
			...
			while(1) {
				...
				if(pstTask->qwFlags & TASK_FLAGS_PROCESS) {
					...
					for(i = 0; i < iCount; i++) {
						...
						// 자식 스레드 리스트에 연결된 정보는 태스크 자료구조에 있는 stThreadLink의 시작 어드레스.
						// 따라서 어드레스를 구하려면 별도의 계산이 요구
						pstChildThread = GETTCBFROMTHREADLINK(pstThreadLink);

						// 다시 자식 스레드 리스트에 삽입하여 해당 스레드가 종료될 대
