...
void kInitializeGDTTableAndTSS(void) {
	...
	//NULL, 64bit Code/Data, TSS를 위한 4개의 segment 생성
...
// 8byte 크기의 GDT 엔트리에 값 설정
// code, data segment descriptor 설정하는데 사용
...
// TSS segment descriptor 설정하는데 사용
...
void kInitializeTSSSegment(TSSSEGMENT* pstTSS) {
	...
	//IO를 TSS의 limit 값보다 크게 설정 -> IO Map을 사용하지 않도록 함
...
// IDT gate descriptor에 값을 설정
void kSetIDTEntry(IDTENTRY* pstEntry, void* pvHandler, WORD wSelector, BYTE bIST, BYTE bFlags, BYTE bType) {
	pstEntry->wLowerBaseAddress = (QWORD)pvHandler & 0xFFFF; //ㅋㅋㅋㅋ 이 시발 이거 0xFF 했더니 handler 안뜸
