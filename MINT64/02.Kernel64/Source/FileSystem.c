...
// 		MINT 파일시스템이라면 파일 시스템에 관련된 각종 정보를 읽어 자료구조에 삽입
BOOL kMount(void) {
	MBR* pstMBR;
...
BOOL kFormat(void) {
	...
	//========================================================================
	// 하드 디스크의 정보를 얻어 하드 디스크의 총 섹터 수 계산
	...
	//========================================================================
	// 계산된 정보를 MBR에 덮어 쓰고, 루트 디렉터리 까지 모두 0으로 초기화하여 fs 생성
	//========================================================================
	// MBR 영역 읽기
	if(gs_pfReadHDDSector(TRUE, TRUE, 0, 1, gs_vbTempBuffer) == FALSE) {
		// 동기화 처리
	...
	for(i = 0; i < (dwClusterLinkSectorCount + FILESYSTEM_SECTORSPERCLUSTER); i++) {
		// 루트 디렉터리(클러스터 0)는 이미 fs이 사용하고 있으므로, 할당된 것으로 표시
		if(i == 0) {
...
// 		빈 캐시 버퍼가 없는 경우 오래된 것 중에 하나를 골라 비운 후 사용
...
static BOOL kInternalWriteClusterLinkTableWithCache(DWORD dwOffset, BYTE* pbBuffer) {
	...
	// 캐시 버퍼에 없다면 캐시 버퍼를 할당받아서 캐시 내용 갱신
	pstCacheBuffer = kAllocateCacheBufferWithFlush(CACHE_CLUSTERLINKTABLEAREA);
	if(pstCacheBuffer == NULL) {
		return FALSE;
	}
...
static BOOL kSetClusterLinkData(DWORD dwClusterIndex, DWORD dwData) {
	...
	// 한 섹터에 128개의 클러스터 링크가 들어가므로 128로 나누면 섹터 오프셋을 구할 수 잇음
...
BOOL kGetDirectoryEntryData(int iIndex, DIRECTORYENTRY* pstEntry) {
	...
	// 파일시스템을 인식하지 못했거나 인덱스가 올바르지 않으면 실패
	if((gs_stFileSystemManager.bMounted == FALSE) || (iIndex < 0) || (iIndex >= FILESYSTEM_MAXDIRECTORYENTRYCOUNT)) {
		return FALSE;
	}
...
// 루트 디렉터리에서 파일 이름이 일치하는 엔트리를 찾아서 인덱스 반환
...
// 파라미터로 넘어온 클러스터부터 파일의 끝까지 연결된 클러스터를 모두 반화
...
DWORD kWriteFile(const void* pvBuffer, DWORD dwSize, DWORD dwCount, FILE* pstFile) {
	...
	while(dwWriteCount != dwTotalCount) {
		...
		// 여러 클러스터에 걸쳐져 있따면 현재 클러스터에서 남은 만큼 쓰고 다음 클러스터로 이동
	...
	//=======================================================================
	// 파일 크기가 변했다면 루트 디렉터리에 잇는 디렉터리 엔트리 정보를 갱신
	//=======================================================================
