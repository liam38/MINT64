...
void kSetCursor(int iX, int iY) {
	...
	// CRTC 컨트롤 addr reg(0x3D4)에 0x0E 전송
	// 상위 커서 위치 reg 선택
	kOutPortByte(VGA_PORT_INDEX, VGA_INDEX_UPPERCURSOR);
	// CRTC 컨트롤 data reg(0x3D5)에 커서의 상위 바이트 출력
	kOutPortByte(VGA_PORT_DATA, iLinearValue >> 8);

	// CRTC 컨트롤 addr reg(0x3D4)에 0x0F 전송
	// 하위 커서 위치 reg 선택
	kOutPortByte(VGA_PORT_INDEX, VGA_INDEX_LOWERCURSOR);
	// CRTC 컨트롤 data reg(0x3D5)에 커서의 하위 바이트 출력
...
void kGetCursor(int* piX, int* piY) {
	// 저장된 위치를 콘솔 화면의 너비로 나눈 나머지로 X 좌표 획득
	// 화면 너비로 나누면 Y 좌표 획득
...
// \n, \t 같은 문자가 포함된 문자열을 출력한 후 화면상의 다음 출력 위치 반환
