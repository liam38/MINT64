...
BOOL kWaitForACKAndPutOtherScanCode(void) {
	...
	// ACK가 오기 전에 키보드 출력버퍼(0x60)에 키 데이터가 저장될 수 있으므로
	// 키보드에서 전달된 데이터를 최대 100개까지 수신하여 ACK를 확인
	for(j = 0; j < 100; j++) {
		// 0xFFFF만큼 루프를 수행할 시간이면 충분히 커맨드의 응답이 올 수 있음
		// 0xFFFF루프를 수행한 이후에도 출력버퍼(0x60)가 차 있지 않으면 무시하고 읽음
		for(i = 0; i < 0xFFFF; i++) {
			// 출력버퍼(0x60)가 차 잇으면 데이터를 읽을 수 있음
			if(kIsOutputBufferFull() == TRUE)
				break;
		}

		// 출력버퍼(0x60)에서 읽은 데이터가 ACK(0xFA)면 성공
